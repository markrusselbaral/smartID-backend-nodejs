...
class PortService {
  ...
  async reloadPorts(id, ports) {
    try {
        const response = await axios.get(`${process.env.NODE_BASE_URL}/reload-ports/${id}`, { params: { ports: ports } });
  ...
  async updatePortActiveStatus(portId, kioskId) {
      try {
          const response = await axios.get(`${process.env.NODE_BASE_URL}/turn-on-port/${portId}/${kioskId}`);
